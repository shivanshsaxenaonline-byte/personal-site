...
- Went somewhere.`}
  ...
  /* Driven off the real section list rather than named sections. The
     upstream version hardcoded "Some History" and "Travel / Geography", so
     rewriting the About page — the first thing any fork does — broke it. */
  it('renders matching hash links and heading ids into static markup', () => {
    const html = renderToStaticMarkup(
      <AboutContent markdown={aboutMarkdown} />,
    );

    const titles = getActualSectionTitles(aboutMarkdown);
  ...
  it('supports same-page hash navigation from section links', async () => {
    ...
    const [firstTitle, ...otherTitles] = getActualSectionTitles(aboutMarkdown);
    const lastTitle = otherTitles.at(-1) ?? firstTitle;
    ...
    await waitFor(() => {
      expect(window.location.hash).toBe(`#${createHeadingId(firstTitle)}`);
    });
    expect(document.querySelector(window.location.hash)).toHaveTextContent(
      firstTitle,
    );

    const heading = screen.getByRole('heading', { name: lastTitle });
    const permalink = within(heading).getByRole('link', { name: lastTitle });

    permalink.click();

    await waitFor(() => {
      expect(window.location.hash).toBe(`#${createHeadingId(lastTitle)}`);
    });
    expect(document.querySelector(window.location.hash)).toHaveTextContent(
      lastTitle,
    );
