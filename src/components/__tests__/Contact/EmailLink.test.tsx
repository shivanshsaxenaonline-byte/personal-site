  ...
  it('never blanks or snaps back to the address mid-animation', () => {
    ...
    for (let elapsed = 0; elapsed < 120_000; elapsed += 50) {
      ...
      /* The flash is a *jump* to the complete address from some other alias.
         ...
         a frame that completes the address is only suspicious when the frame
         before it was not a prefix of the address.

         The previous version guarded on `previous.length > 1` instead, which
         only held because the upstream address had a two-character local part
         ("hi"). Any longer one made its own legitimate final keystroke look
         like a flash. */
      if (shown === localPart && !localPart.startsWith(previous)) {
