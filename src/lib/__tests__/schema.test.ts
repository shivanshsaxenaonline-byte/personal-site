...
describe('personNode', () => {
  ...
  it('uses author name and split given/family names', () => {
    ...
    const [given, ...rest] = AUTHOR_NAME.split(' ');
    expect(node.givenName).toBe(given);
    expect(node.familyName).toBe(rest.join(' '));
  });
...
describe('site image metadata', () => {
  it('keeps declared image dimensions in sync with the public asset', () => {
    /* Read through the shared parser rather than a JPEG-only reader kept in
       this file. The portrait is whatever format its owner supplies — the
       placeholder is a PNG — and `readImageSize` handles PNG, JPEG, GIF, and
       WebP, so swapping the photograph does not mean editing a test. */
