...
describe('getWritingItems', () => {
  /* Whether there is any *external* writing is a content decision, so this
     asserts the merge and the ordering, not that both kinds exist. At least
     one local post is guaranteed: the build needs one for
  ...
  it('merges published local posts and external writing newest first', () => {
    ...
    expect(items.every((item) => typeof item.isExternal === 'boolean')).toBe(
      true,
    );
