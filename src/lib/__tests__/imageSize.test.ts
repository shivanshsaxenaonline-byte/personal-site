...
describe('readImageSize', () => {
  ...
  /* There was a third case here reading a real .webp article asset off disk.
     It was removed with the upstream author's posts, and re-adding one only to
     satisfy a test would put an undisplayable file in `public/`. The coverage
     it carried is kept: both WebP encodings are exercised by the synthetic
     `extendedWebp`/`lossyWebp` buffers in the `parseImageSize` block above,
     and the real-file read path is covered by the `/og.png` case. */
