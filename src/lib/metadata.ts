...
/**
 * `site` and `creator` are only meaningful when there is a handle to name.
 * Emitting them empty produced `<meta name="twitter:site" content="">`, which
 * validators flag, so the pair is spread in only when `TWITTER_HANDLE` is set.
 */
