...
- **Static export**: `output: 'export'`—no server features. Deployed to Vercel, which serves the export directly
...
- **Portrait asset**: Use JPEG for the photograph. `ThemePortrait` reads `SITE_IMAGE_PATH` from `src/lib/utils.ts`; keep `SITE_IMAGE_DIMENSIONS` in that file equal to the actual image header dimensions, as checked by `src/lib/__tests__/schema.test.ts`. The placeholder generator has been retired.
...
- **Writing topic copy**: `WRITING_DESCRIPTION` in `src/lib/schema.ts` is shared by the Writing introduction, page metadata, Blog schema, and RSS description. Keep it aligned with the published content instead of copying the upstream author's subject areas into separate strings.
- **Project case studies**: `src/data/projects.ts` supplies the homepage, project index, detail routes, diagrams, and sitemap. Keep slugs stable; distinguish `website` from `source` links and proposed `future` ideas from implemented features. Do not invent missing stack details, public URLs, or outcome metrics.
- **Project artwork**: Use the generated raster illustrations in `public/images/projects/`, with alt text, captions, and actual dimensions in each project's `image` metadata. Preserve the prompt/source notes in `docs/project-image-prompts.md`. These are conceptual use-case images, not screenshots; do not replace them with CSS diagrams.
- **Design system**: Three type roles, and every element should pick one deliberately: `--font-display` (Space Grotesk) for names and headings, `--font-body` (Source Serif 4) for prose, and `--font-mono` (JetBrains Mono) for data, dates, labels, and buttons. Structure is carried by hairlines (`--rule`) and near-square radii, not by shadow and float. A heavy `--color-fg-bold` rule opens a section; a `--color-border` hairline divides within one
