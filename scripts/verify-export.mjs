...
/**
 * `twitter:site` and `twitter:creator` are deliberately absent from this list.
 * Both name an account handle, and a site whose owner has no X account should
 * not be forced to emit empty ones — `src/lib/metadata.ts` omits the pair when
 * `TWITTER_HANDLE` is blank. Set that constant and they appear; either way the
 * card still renders, because `twitter:card` and the image are required above.
 */
