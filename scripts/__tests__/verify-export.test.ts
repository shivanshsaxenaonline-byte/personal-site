...
describe('verify-export', () => {
  ...
  /* `twitter:creator` used to be in this list. It names an account handle,
     which a site whose owner has no X account should not be forced to emit —
     see the note on REQUIRED_SOCIAL_META in `scripts/verify-export.mjs`.
     `twitter:image` takes its place here: it is required, and unlike the
     handle it is always present. */
  it.each([
